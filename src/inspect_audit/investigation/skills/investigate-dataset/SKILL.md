...
description: "How to look at the items themselves rather than the code that loads them: raw schema, missing fields, duplicates, distributions, and what record_to_sample does to each record on the way to a sample. Read it when a question is about the data."
---
...
The dataset loading code is in the read-only snapshot at the seed's `snapshot` path,
`/inputs/source` when there is one. Downloading
...
and the difference is worth a check either way: an item's text in the log is what the
model saw, and an item's text on HuggingFace is what the benchmark claims to be.

Ignore `uv run`, `tests/utils/`, and anything that assumes an `inspect_evals` checkout.
...
python -c "from datasets import load_dataset_builder; b = load_dataset_builder('org/name'); print(b.info)"

# List available configs/subsets
python -c "from datasets import get_dataset_config_names; print(get_dataset_config_names('org/name'))"

# List available splits
python -c "from datasets import load_dataset; print(load_dataset('org/name', split=None).keys())"
```
