...
HuggingFace metadata can be read from the Dataset Viewer API without downloading anything (`https://datasets-server.huggingface.co/info?dataset=<path>`, or with curl). In an `inspect_evals` checkout the same thing is wrapped as `tests/utils/huggingface.py`, which is not present in a task-scoped snapshot:
