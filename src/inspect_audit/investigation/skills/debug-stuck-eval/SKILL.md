...
description: "Why a Hawk job is slow, stuck or empty: trouble reasons, per-sample phases, retries, limits, and which of them are the job's problem rather than the benchmark's. Read it when a job you launched is not progressing."
---


## In this container
This is the skill to read when one of your jobs is slow, stuck, or has produced nothing.
...
You have no auth step (the operator is already logged in), no `hawk watch -f` streaming
(`jobs(action="watch")` is one snapshot; call it again), and no ability to edit or
resubmit a running job: to change something you stop it and submit a corrected config.
The `hawk trace --full --raw > trace.log` and `inspect trace anomalies` route is not
available; the trace tail is.

The vocabulary is the part to keep. `Stuck` means Kubernetes reported trouble while pods
are alive. The trouble reasons and what each implies: runner out of memory, image pull
failing, container crash looping, pods can't be scheduled. On a **sandbox** pod, OOM and
crash-looping are per-sample and Inspect retries them; only runner-pod trouble is
job-level. Per-sample phases are waiting, init, running, scoring, completed, errored,
limit, canceled, and `limit: token` means a sample halted on a limit you set, which is
