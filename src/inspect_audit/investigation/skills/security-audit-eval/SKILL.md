...
This skill produces a security audit of an Inspect AI evaluation, with a clear verdict (safe / safe-with-caveats / unsafe) backed by file:line evidence.
...
## In this container
This was written to vet a third-party eval before running it on someone's laptop, so it
talks about a decision you are not making: the benchmark you are auditing has already
been run, and running it again happens on Hawk in a disposable pod, not here. Read it
for its threat model, not its verdict.

The eight categories are a checklist for two of your report's sections. Host-side code
that executes at load or score time, externally fetched files that nobody
quality-controls, a sandbox that shares the host network or mounts the host filesystem,
credentials reachable from inside the sandbox, and unpinned dependencies are all facts
about **The harness and environment**; sloppiness that is not exploitable is **How it is
built**. A sandbox weak enough that a model could reach the grader, the gold answers or
another sample's state is not just a security finding, it is a validity finding: say
what it lets a model do to its own score.
...
Write ``/workspace/report/findings.json`` with this structure:
